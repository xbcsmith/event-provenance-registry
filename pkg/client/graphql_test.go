...
func customStringCompare(query string, options []string) cmp.Comparison {
	return func() cmp.Result {
		...
		return cmp.ResultFailure(
			fmt.Sprintf("%q did not match available options %v", query, options),
		)
	}
}
...
func TestNewGraphQLSearchRequest(t *testing.T) {
	...
	expected := []string{
		`query ($obj: FindEventInput!){events(event: $obj) {id,name,version,release,platform_id,package,description,success,event_receiver_id}}`,
	}
	...
	expected = []string{
		`query ($obj: FindEventReceiverInput!){event_receivers(event_receiver: $obj) {id,name,type,version,description}}`,
	}
	req = NewGraphQLSearchRequest(operation, params, fields)
	assert.Check(t, customStringCompare(req.Query, expected))
	...
	expected = []string{
		`query ($obj: FindEventReceiverGroupInput!){event_receiver_groups(event_receiver_group: $obj) {id,name,type,version,description}}`,
	}
	req = NewGraphQLSearchRequest(operation, params, fields)
	assert.Check(t, customStringCompare(req.Query, expected))
}
...
func TestNewGraphQLMutationRequest(t *testing.T) {
	...
	expected = []string{
		`mutation ($obj: CreateEventReceiverInput!){create_event_receiver(event_receiver: $obj)}`,
	}
	req = NewGraphQLMutationRequest(operation, params)
	assert.Check(t, customStringCompare(req.Query, expected))
	...
	expected = []string{
		`mutation ($obj: CreateEventReceiverGroupInput!){create_event_receiver_group(event_receiver_group: $obj)}`,
	}
	req = NewGraphQLMutationRequest(operation, params)
	assert.Check(t, customStringCompare(req.Query, expected))
}
