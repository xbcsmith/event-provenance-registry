...
func TestGetSASLCredentials(t *testing.T) {
	t.Run(
		"validEnabledSASLSCRAM",
		testCredentials(t, "user", "pass", "SCRAM", "user", "pass", SCRAM, true),
	)
	t.Run(
		"validEnabledSASLSPLAIN",
		testCredentials(t, "user", "pass", "PLAIN", "user", "pass", PLAIN, true),
	)
	t.Run(
		"validDisabledSASLSOAUTH2",
		testCredentials(t, "user", "pass", "OAUTH2", "user", "pass", OAUTH2, true),
	)

	t.Run(
		"invalidDisabledSASLSOAUTH2",
		testCredentials(t, "user", "pass", "garbo", "user", "pass", NONE, false),
	)
...
func testCredentials(
	_ *testing.T,
	setUser, setPass, setAuth, expectedUser, expectedPass string,
